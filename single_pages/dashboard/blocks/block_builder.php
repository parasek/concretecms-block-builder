    ...
    <form method="post" action="<?php echo $formAction; ?>">
        ...
        <div class="ccm-tab-content active" id="ccm-tab-content-block-settings" style="display: none;">
            ...
            <div class="mb-4 <?php in_array('excludedFromRemoval', $fieldsWithError) ? print 'has-error' : false; ?>">
                <?php echo $form->label('excludedFromRemoval', t('Excluded files and folders when rebuilding block')); ?>
                <?php echo $form->textarea('excludedFromRemoval', implode(PHP_EOL, $excludedFromRemoval), ['style' => 'min-height: 200px;']); ?>
                <div class="form-text">
                    <?= t('You can use this field to exclude your own files/folders when rebuilding block (for example: you do not want to delete "templates" folder).'); ?>
                    <br>
                    <?= t('You can exclude first level files/folders only.'); ?>
                    <br>
                    <?= t('Do not exclude standard files/folders generated by Block Builder.'); ?>
                    <br>
                    <?= t('Every path should be relative to controller.php (for example: templates) and put in new line.'); ?>
                </div>
            </div>
