...
readonly class CreateBlockDtoFactory
{
    ...
    public function fromArray(array $data): CreateBlockDto
    {
        $environmentDto = $this->environmentService->getEnvironment();
        $isBlockBeingGenerated = $this->isBlockBeingGenerated($data);
        ...
        $basic = $this->transformFieldsToDto($data['basic'] ?? []);
        $entries = $this->transformFieldsToDto($data['entries'] ?? []);

        return new CreateBlockDto(
            ...
            fieldsDivider: trim($data['fieldsDivider'] ?? ''),
            entryFieldsDivider: trim($data['entryFieldsDivider'] ?? ''),
            registerViewAssetsCustomCode: trim($data['registerViewAssetsCustomCode'] ?? ''),
            viewCustomCode: trim($data['viewCustomCode'] ?? ''),
            customControllerMethods: trim($data['customControllerMethods'] ?? ''),
    ...
    /**
     * When building/rebuilding a block, there is no "createdAt" field in the data array.
     * This way we can differentiate if a block is being generated (inserting DTO into JSON file)
     * or just displayed (listing existing configs/predefined configs).
     */
    private function isBlockBeingGenerated(array $data): bool
    {
