...
document.addEventListener('DOMContentLoaded', () => {
    const blockBuilder = (() => {
        ...
        const autoGenerateBlockHandle = (blockNameElement) => {
            ...
            const blockHandleElement = blockIdentity.querySelector('[data-block-handle]');
            if (!blockHandleElement) return;

            const handleWasAutoGenerated = blockIdentity.dataset.handleGenerated === '1';
            if (blockIdentity.dataset.handleGenerated === '0') return;
            if (String(blockHandleElement.value || '').trim() !== '' && !handleWasAutoGenerated) return;

            const snakeCasedHandle = toSnakeCase(blockNameElement.value);
            blockHandleElement.value = snakeCasedHandle;
            blockIdentity.dataset.handleGenerated = '1';
        };

        const stopAutoGeneratingBlockHandle = (blockNameElement) => {
            const blockIdentity = blockNameElement.closest('[data-block-identity]');
            if (blockIdentity) blockIdentity.dataset.handleGenerated = '0';
        };

        const restartBlockHandleAutoGenerationWhenEmpty = (blockHandleElement) => {
            if (String(blockHandleElement.value || '').trim() !== '') return;

            blockHandleElement.closest('[data-block-identity]')?.removeAttribute('data-handle-generated');
        };
        ...
        const bindFunctions = () => {
            ...
            bbContainer.addEventListener('input', (e) => {
                if (e.target.matches('[data-block-name-source]')) {
                    autoGenerateBlockHandle(e.target);
                }
            ...
            bbContainer.addEventListener('focusout', (e) => {
                if (e.target.matches('[data-block-name-source]')) return stopAutoGeneratingBlockHandle(e.target);
                if (e.target.matches('[data-block-handle]')) return restartBlockHandleAutoGenerationWhenEmpty(e.target);
