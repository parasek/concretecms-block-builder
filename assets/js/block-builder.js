...
document.addEventListener('DOMContentLoaded', () => {
    const blockBuilder = (() => {
        ...
        const autoGenerateHandle = (input) => {
            ...
            if (String(handleElement.value || '').trim() !== '') return;
            if (entry.dataset.handleGenerated === '1') return;

            const camelCasedHandle = toCamelCase(input.value);
            if (String(camelCasedHandle || '').trim() === '') return;

            handleElement.value = camelCasedHandle;
            entry.dataset.handleGenerated = '1';
        };

        const resetAutoGeneratedHandleFlag = (input) => {
            if (input.value === '') {
                input.closest('[data-entry]')?.setAttribute('data-handle-generated', '0');
            }
        };
        ...
        const bindFunctions = () => {
            ...
            bbContainer.addEventListener('focusout', (e) => {
                ...
                if (e.target.matches('[data-entry-title-source]')) return autoGenerateHandle(e.target);
                if (e.target.matches('[data-entry-handle]')) return resetAutoGeneratedHandleFlag(e.target);
            });
